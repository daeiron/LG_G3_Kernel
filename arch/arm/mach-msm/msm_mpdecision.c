...
unsigned int state_mpdec = MSM_MPDEC_IDLE;
...
static int mp_decision(void) {
	...
	if (state_mpdec == MSM_MPDEC_DISABLED)
				...
				const char *buf, size_t count)
{
	...
	switch (buf[0]) {
	case '0':
		state_mpdec = MSM_MPDEC_DISABLED;
